...
- Secret management
- Rate limiting
- Security audit of Policy Engine (contains experimental, untested code)
- Real ML training (currently uses synthetic data for architecture demo)

**Current state:** Alpha software with experimental features and no security hardening.

</details>

<details>
<summary><strong>Does FLOPY-NET support other FL frameworks?</strong></summary>

<br>

**No, Flower only** (v1.5.0)

The modular architecture would allow framework adapters, but none are implemented. You'd need to:
1. Create adapter in `src/fl/adapters/`
2. Implement common FL interface (`src/core/interfaces/`)
3. Update configuration system
4. Maintain Policy Engine & metrics integration

This is a high-priority contribution opportunity.

</details>

<details>
<summary><strong>Can I run FLOPY-NET without GNS3?</strong></summary>

<br>

**Yes.** Use the root `docker-compose.yml`.
...
**You get:**
- FL Server, Clients, Policy Engine, Collector
- Dashboard and monitoring
- Basic FL experiments

**You lose:**
- Network emulation (latency, packet loss, bandwidth control)
- SDN integration
- Topology visualization
- Network failure injection

**Use case:** Quick testing, development, when network effects aren't your research focus.

</details>

<details>
<summary><strong>What datasets are supported?</strong></summary>

<br>

**Currently: Synthetic data, with MNIST/CIFAR-10 loaders available but not wired up**

**What's implemented:**
- ✅ `FederatedMNIST` and `FederatedCIFAR10` classes in `src/fl/client/data_loader.py`
- ✅ Actual torchvision dataset downloads with `download=True`
- ✅ Federated data partitioning logic (IID and non-IID)
- ✅ PyTorch DataLoader integration
- ❌ **Not connected to training loop** - clients use `np.random.randn()` for parameters

**Current training behavior:**
```python
# In fl_client.py line ~1015-1050:
# Creates realistic parameter arrays using np.random.randn()
params.append(np.random.randn(*weight_shape).astype(np.float32) * 0.1)
# Simulates training time (10-20s) with time.sleep()
# No actual learning occurs
```

**To wire up real datasets:**

```python
# Replace MinimalModelInterface in fl_client.py with:
from src.fl.client.data_loader import create_federated_dataloaders
...
# Then replace time.sleep() simulation with actual PyTorch training
...
**Status:** Infrastructure 80% ready, needs integration work.

</details>

<details>
<summary><strong>How do I add more FL clients?</strong></summary>

<br>

**Steps:**

1. Edit `config/topology/basic_topology.json`:
```json
{
  "nodes": [
    {
...
2. Update `config/scenarios/basic_main.json`:
```json
{
  "network": {
    "ip_map": {
...
3. Ensure resources: 8GB RAM minimum, 16GB recommended (each client ~1GB)

4. Redeploy:
```bash
python src/scenarios/basic/scenario.py --config config/scenarios/basic_main.json
```

**Note:** GNS3 VM resource allocation is critical. Adjust in VMware before adding clients.

</details>

<details>
<summary><strong>Is FLOPY-NET cross-platform?</strong></summary>
...
|----------|--------|-------|
| Windows 10/11 | Fully tested | PowerShell, VMware Workstation Player |
| Linux (Ubuntu) | Should work | Native Docker, KVM for GNS3 VM |
| macOS | Should work | VMware Fusion, some path adjustments |

**Platform differences:**
- Windows: Use PowerShell, backtick for line continuation, `python`
- Linux/macOS: Use bash, backslash continuation, `python3`

</details>

<details>
<summary><strong>How do I export experiment results?</strong></summary>

<br>

**Method 1: Dashboard Export**
1. http://localhost:8085 → Metrics Explorer
2. Select metrics and time range
3. Export → CSV or JSON

**Method 2: Direct database query**
```bash
sqlite3 src/collector/metrics.db
SELECT * FROM training_metrics ORDER BY timestamp DESC;
```

**Method 3: API**
```bash
curl http://localhost:8001/api/fl/rounds > fl_rounds.json
```

**Note:** Metrics collection depends on Collector service running and FL components properly reporting data. Check logs if export is empty.
...
<details>
<summary><strong>How do FL clients communicate with the server?</strong></summary>
...
```
FL Client ←→ FL Server
gRPC (HTTP/2) on port 8080
Protocol Buffers serialization
TCP/IP through GNS3 network
```

**Details:**
- Framework: Flower's gRPC implementation
- Port: 8080 (configurable)
- Serialization: Protocol Buffers
- Connection: Persistent during rounds
- Messages: ClientConnect, FitIns, FitRes, EvaluateIns/Res

All traffic goes through the GNS3 network and experiences configured latency, loss, and bandwidth constraints.

</details>

<details>
<summary><strong>What is the FL communication protocol?</strong></summary>

<br>

**Training round workflow:**

1. **Registration** - Client connects, sends capabilities
2. **Instructions** - Server broadcasts training parameters
3. **Local training** - Client trains on local data
4. **Updates** - Client sends model weights back
5. **Aggregation** - Server aggregates using FedAvg
6. **Distribution** - Server sends updated global model

This repeats for N rounds (typically 5-100).

**Formula:** `w_global = Σ(n_k/n_total × w_k)`

Policy Engine validates actions at each step.

</details>

<details>
<summary><strong>Can I use real ML models and datasets?</strong></summary>

<br>

**Current state: Synthetic data with real dataset loaders available but not wired up**
...
1. Wire up data loaders in `src/fl/client/fl_client.py`:
```python
# Replace MinimalModelInterface with:
from src.fl.client.data_loader import create_federated_dataloaders
...
2. Replace synthetic training simulation (line ~1062-1130 in `fl_client.py`)
3. Use actual PyTorch model from `src/fl/common/models/`

**Why synthetic currently?**
- Demonstrates federated architecture without ML dependencies
- Faster experiments (10-20s vs minutes)
- No 500MB+ dataset downloads
- Separates network research from ML tuning

**Implementation status:** Infrastructure 80% complete, integration 20% complete.

</details>

<details>
<summary><strong>Why does training use random data?</strong></summary>

<br>

**Design decision:** Platform architecture over ML research

**Benefits:**
- Focus on network effects, policy enforcement, distributed systems
- No dataset downloads (faster setup)
- Predictable resource usage
- Easier debugging
- Reduced dependencies

**For real ML:** The infrastructure exists but requires manual integration. See `src/fl/client/data_loader.py` for MNIST/CIFAR-10 classes that need to be wired into the training flow.

Think of FLOPY-NET as the infrastructure; you provide the intelligence (models + data).
...
<details>
<summary><strong>Why use SDN in federated learning?</strong></summary>

<br>

**SDN = Software-Defined Networking** enables programmable network control

**Benefits for FL research:**

- **Dynamic QoS** - Prioritize FL traffic vs background traffic
- **Policy enforcement** - Bandwidth/latency limits per client
- **Traffic monitoring** - Real-time bandwidth usage, packet loss
- **Failure injection** - Drop connections, simulate congestion
- **Topology control** - Change network paths during training

**Research questions:**
- How does packet loss affect convergence?
- Impact of asymmetric bandwidth?
- Can FL adapt to dynamic conditions?

**Note:** SDN implementation is experimental. Some features may not work as expected.

</details>

<details>
<summary><strong>What does the SDN Controller do?</strong></summary>

<br>

**Role:** Programs OpenFlow switches for network control

**Stack:**
- Framework: Ryu (Python)
- Protocol: OpenFlow 1.3
- Ports: 6633 (OpenFlow), 8181 (REST API)
...
**Status:** Implemented but **testing is limited**. Complex policy scenarios may not work reliably.

</details>

<details>
<summary><strong>Can I run FL without SDN?</strong></summary>
...
- ✅ Experiment with client selection
- ✅ Use Docker Compose or basic GNS3 networking

**With SDN:**
- Network-aware FL research
- Programmable QoS
- Policy-driven routing
- Advanced failure injection

**To skip SDN:** Don't deploy the SDN controller node in your topology.

</details>

<details>
<summary><strong>What network conditions can I simulate?</strong></summary>

<br>

**Supported parameters:**

1. **Latency** - Fixed (50ms) or variable (10-100ms)
2. **Bandwidth** - 56Kbps to 10Gbps
3. **Packet loss** - 0-100%
4. **Jitter** - Delay variation
5. **Link failures** - Intermittent disconnections

**Example scenarios:**

| Network | Latency | Bandwidth | Loss |
|---------|---------|-----------|------|
| 5G | 10-30ms | 100+ Mbps | <1% |
| WiFi | 20-50ms | 10-100 Mbps | 2-5% |
| Satellite | 500-700ms | 1-10 Mbps | 0-5% |

Configure in topology JSON. Use SDN for dynamic changes during training.

**Note:** Accuracy of emulation depends on GNS3 configuration and VM resources.

</details>

<details>
<summary><strong>GNS3 vs Docker Compose deployment?</strong></summary>
...
**Pros:** Simple, fast, low resources, easy debugging
**Cons:** No network emulation, flat topology, no SDN

**Use for:** Development, quick testing, learning
...
**Pros:** Network emulation, configurable conditions, SDN, visual topology
**Cons:** Complex setup, high resources, slower, harder to debug

**Use for:** Network research, realistic testing

**Recommendation:** Start with Docker Compose, move to GNS3 when network effects matter.
...
<details>
<summary><strong>How does Policy Engine affect FL training?</strong></summary>

<br>

**Role:** Centralized decision authority for FL system

**Functions:**

1. **Client selection** - Decides which clients participate (based on trust, resources)
2. **Model validation** - Checks model size, detects anomalies
3. **Security rules** - Flags Byzantine attacks, privacy violations
4. **Compliance** - Enforces GDPR, data residency

**Integration:**
- FL Server queries before client selection and aggregation
- FL Clients query before sending updates
- SDN Controller queries for routing decisions

**Status:** ⚠️ **Policy Engine is experimental and contains untested policy types**. Many enforcement mechanisms are not fully implemented or reliable. Use with caution.
...
<details>
<summary><strong>How do I debug FL communication issues?</strong></summary>
...
4. **Policy Engine**
```bash
curl -X POST http://192.168.100.20:5000/evaluate
```
...
6. **Capture traffic in GNS3**
- Right-click link → Start capture
- Analyze in Wireshark (filter: `tcp.port == 8080`)

**Common issues:**

| Symptom | Cause | Fix |
|---------|-------|-----|
| "Connection refused" | Server not started | Check container running |
| "No route to host" | Network misconfigured | Verify GNS3 links, IPs |
| "Timeout" | High latency/firewall | Check settings, resources |
| "Policy denied" | Policy rejection | Check policy logs |

</details>

<details>
<summary><strong>How scalable is FLOPY-NET?</strong></summary>

<br>

**Current scale:** 2-10 FL clients (network research, not production)

**Resource requirements:**

| Clients | GNS3 VM RAM | GNS3 VM CPU | Time |
|---------|-------------|-------------|------|
| 2-3 | 8 GB | 4 cores | 5-10 min |
| 4-6 | 16 GB | 6 cores | 10-20 min |
| 7-10 | 24-32 GB | 8 cores | 20-40 min |

**Bottlenecks:**
- GNS3 VM resources (~1GB per client)
- Network emulation overhead
- Single VM limit
...
**Designed for:**
- Network research (2-10 clients with deep analysis)
- Protocol testing
- Algorithm development under network constraints

**Scale comparison:** Google FL has 1000-100,000+ devices; FLOPY-NET focuses on 2-10 with detailed network emulation.
